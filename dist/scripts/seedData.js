...
const ScentService_1 = __importDefault(require("../services/ScentService"));
const categoryTypes = {
    Box: 1,
    Card: 2,
    Color: 3,
...
async function seed() {
    ...
    // Clear all
    await data_source_1.AppDataSource.query('TRUNCATE "box", "card", "color", "mold", "scent", "category" RESTART IDENTITY CASCADE');
    // Seed categories
    const categories = await categoryRepo.save(categoriesSeed);
    const getCategoryId = (name, entityType) => {
        const found = categories.find(c => c.name === name && c.entityType === entityType);
        if (!found)
            throw new Error(`Category not found for ${name} - ${entityType}`);
        return found.id;
    };
    // Seed boxes
    for (const box of boxesSeed) {
        await BoxService_1.default.getInstance().createBox({ ...box, categoryId: getCategoryId(box.categoryName, categoryTypes.Box) });
    }
    // Seed cards
    for (const card of cardsSeed) {
        await CardService_1.default.getInstance().createCard({ ...card, categoryId: getCategoryId(card.categoryName, categoryTypes.Card) });
    }
    // Seed colors
    for (const color of colorsSeed) {
        await ColorService_1.default.getInstance().createColor({ ...color, categoryId: getCategoryId(color.categoryName, categoryTypes.Color) });
    }
    // Seed molds
    for (const mold of moldsSeed) {
        await MoldService_1.default.getInstance().createMold({ ...mold, categoryId: getCategoryId(mold.categoryName, categoryTypes.Mold) });
    }
    // Seed scents
    for (const scent of scentsSeed) {
        await ScentService_1.default.getInstance().createScent({ ...scent, categoryId: getCategoryId(scent.categoryName, categoryTypes.Scent) });
    }
